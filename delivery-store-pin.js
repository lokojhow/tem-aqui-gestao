...
function loadMobileOrderActions(){if(document.getElementById('mobileOrderActionsLoader'))return;const s=document.createElement('script');s.id='mobileOrderActionsLoader';s.src='./mobile-order-actions.js?v=1.0.1';s.defer=true;document.body.appendChild(s);}
function boot(){loadMobileOrderActions();refresh();document.addEventListener('click',e=>{if(e.target.closest?.('[data-open-order],#ordersRefresh,[data-set-status]'))setTimeout(refresh,250);},true);clearInterval(timer);timer=setInterval(refresh,5000);}
