(() => {
  ...
  loadCss('barcode-scanner.css?v=5');
  ...
  function enhanceCategoryDialog(){
    ...
    if(!form.querySelector('.dialog-close-x')){
  ...
  function improveNewProduct(){
    ...
    btn.addEventListener('click',()=>setTimeout(()=>document.getElementById('productEditor')?.scrollIntoView({behavior:'smooth',block:'start'}),120));
  }
  ...
  function enhanceProductImage(){
    ...
    box.innerHTML='<div class="product-image-preview"><span>Sem imagem</span></div><label class="product-image-upload">📷 Escolher foto<input type="file" accept="image/*" capture="environment"></label><small>Você pode tirar uma foto ou escolher da galeria.</small>';
    ...
    box.querySelector('input[type="file"]').addEventListener('change',async e=>{
      ...
      catch(err){console.error(err);alert('Não foi possível preparar esta imagem.');}
    });
  }
