const CACHE='tem-aqui-gestao-v0-9-4-storefront1';
const CORE=['./','./index.html','./styles.css','./responsive.css','./product-uniform.css','./header-cleanup.css','./barcode-scanner.css','./storefront-manager.css','./pos-enhancements.js','./barcode-scanner.js','./storefront-manager.js','./app.js','./manifest.json','./supabase-config.js','./gestao-backend.js','./logo-tem-aqui-gestao.png','./icon-192.png','./icon-512.png'];
...
self.addEventListener('fetch',e=>{
  ...
  if(u.pathname.endsWith('/styles.css')){
    ...
    ]).then(([base,responsive,uniform,cleanup,barcode,storefront])=>{
      const nr=new Response(base+'\n'+responsive+'\n'+uniform+'\n'+cleanup+'\n'+barcode+'\n'+storefront,{status:200,headers:{'Content-Type':'text/css; charset=utf-8','Cache-Control':'no-store'}});
  ...
  if(u.pathname.endsWith('/app.js')){
    ...
    ]).then(([base,enh,barcode,storefront])=>{
      const nr=new Response(base+'\n'+enh+'\n'+barcode+'\n'+storefront,{status:200,headers:{'Content-Type':'text/javascript; charset=utf-8','Cache-Control':'no-store'}});
      caches.open(CACHE).then(c=>c.put(e.request,nr.clone()));
      return nr;
    }).catch(()=>caches.match(e.request)));
    return;
