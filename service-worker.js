const CACHE='tem-aqui-gestao-v0-9-4-responsive1';
const CORE=['./','./index.html','./styles.css','./responsive.css','./app.js','./manifest.json','./supabase-config.js','./gestao-backend.js','./logo-tem-aqui-gestao.png','./icon-192.png','./icon-512.png'];
...
self.addEventListener('fetch',e=>{
  ...
  if(u.pathname.endsWith('/styles.css')){
    ...
    ]).then(([base,responsive])=>{
      const nr=new Response(base+'\n'+responsive,{status:200,headers:{'Content-Type':'text/css; charset=utf-8','Cache-Control':'no-store'}});
